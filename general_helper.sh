...
findIpsInDomains()
{
  ...
  input_file="$1"
  # Read the contents of the file and search for lines that contain an IP address in the format "X.X.X.X" or "X-X-X-X", where X is a number between 0 and 255.
  cat "$input_file" | grep -E '[0-9]{1,3}([.-][0-9]{1,3}){3}'
}
